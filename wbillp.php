...
// database connection
     ...
     elseif (empty($_POST['block']) || empty($_POST['houseno']) || empty($_POST['firstname']) || empty($_POST['lname']) || empty($_POST['phoneno']) || empty($_POST['rrno']) || empty($_POST['prevreading']) || empty($_POST['currentreading']) || empty($_POST['consumption']) || empty($_POST['amount']) ) 
          {
      ...
      {
      	$stmt=$conn->prepare("insert into waterbill(block,houseno,firstname,lname,phoneno,rrno,prevreading,currentreading,consumption,amount)values('".$block."', '". $houseno."', '".$firstname."', '".$lname."', '".$phoneno."', '".$rrno."', '".$prevreading."', '".$currentreading."', '".$consumption."' , '".$amount."')");
      	$stmt->execute();
